...
export const incerateQuantity = (foodId) => async (dispatch) => {
  try {
    let cartData = { cart: [] };
    if (localStorage.zomatoCart) {
      const { cart } = JSON.parse(localStorage.getItem("zomatoCart"));
    ...
    localStorage.setItem("zomatoCart", JSON.stringify({ cart: cartData.cart }));
    return dispatch({ type: INCERATE_QUANTITY, payload: cartData.cart });
...
export const decrementQuantity = (foodId) => async (dispatch) => {
  try {
    ...
    localStorage.setItem("zomatoCart", JSON.stringify({ cart: cartData.cart }));
    return dispatch({ type: DECORATE_QUANTITY, payload: cartData.cart });
  } catch (error) {
    return dispatch({ type: "ERROR", payload: error });
  }
};
